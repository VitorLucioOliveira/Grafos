...
using Segmentation = unordered_map<Vertex, vector<Vertex>>;
...
class UnionFind {
	...
	vector<int> parent;
	vector<float> maxWeight; // Store the maximum weight for each set root
	size_t sets = 0;
...
vector<pair<int, int>> getNeighbors(int i, int j, const vector<vector<<Pixel>>>& m, bool eightConnected = false);
float dissimilarity(const Pixel& a, const Pixel& b);
Graph createGraph(const vector<vector<<Pixel>>>& m, const bool& eightConnected = false);
void saveSegmentedImage(const string& outputFilename, const Segmentation& segment, vector<vector<<Pixel>>>& image);
...
Segmentation ImageSegmentation(const int K, const vector<vector<<Pixel>>>& m) {

	int n = m.width * m.height;

	// NOTE: if eightConnected * 8 else * 4
	vector<pair<pair<Vertex, Vertex>, float>> sortedEdges(n * 8);
...
int main() {
	...
	cout << "0" << endl;
	vector<vector<<Pixel>>> image = loadPPM("inputImages/cavaloGray", 0.8);
...
vector<pair<int, int>> getNeighbors(int i, int j, const vector<vector<<Pixel>>>& m, bool eightConnected) {

	static const vector<pair<int, int>> directions4 = {
		{-1, 0}, {0, -1}, {0, 1}, {1, 0}
	};

	static const vector<pair<int, int>> directions8 = {
...
Graph createGraph(const vector<vector<<Pixel>>>& m, const bool& eightConnected) {
...
void saveSegmentedImage(const string& outputFilename, const Segmentation& segment, vector<vector<<Pixel>>>& image) {
	...
	// Paint the pixels
	vector<Pixel> color = generateColors(segment.size());
